  ...
  <el-card style="padding: 10px;margin: 20px">
    <el-row >
      <div class="demo-input-size" style="padding: 10px">
        <!--      输入框-->
        <span class="demonstration">店铺名：</span>
        <el-input
            v-model="input2"
            class="w-50 m-2"
            placeholder="请输入店铺名"
    ...
    <el-table :data="tableData" style="width: 100%">
      <el-table-column fixed prop="id" label="店铺ID" width="400" />
      <el-table-column fixed prop="storeName" label="店铺名" width="400" />
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  try {
    ...
    // 发送 POST 请求到后端 API
    const response = await request.post('/SellerAddStore', formData);
